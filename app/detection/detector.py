...
class PlateDetector:
    ...
    def detect_by_contours(self, frame: np.ndarray) -> list[tuple[np.ndarray, tuple[int, int, int, int]]]:
        """
        Fallback: find plate-like rectangles using contour detection.
        Used when YOLO misses the plate.
        Returns list of tuples: (cropped plate image, bounding box).
        ...
        for c in contours:
            ...
            if len(approx) == 4:
                ...
                if 1.5 <= ratio <= 7.0 and w > 60:
                    plates.append((frame[y:y+h, x:x+w], (x, y, x + w, y + h)))
