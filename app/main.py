...
async def lifespan(app: FastAPI):
    ...
    # camera processors
    if entry_processor.rtsp_url:
        asyncio.create_task(entry_processor.start())
    else:
        logger.warning("Entry camera RTSP URL is not configured; skipping entry stream")

    if exit_processor.rtsp_url:
        asyncio.create_task(exit_processor.start())
    else:
        logger.warning("Exit camera RTSP URL is not configured; skipping exit stream")
...
@app.get("/stream")
async def stream(camera: str = "entry"):
    rtsp_url = None
    if camera == "entry":
        rtsp_url = config.ENTRY_CAMERA_RTSP
    elif camera == "exit":
        ...
        raise HTTPException(status_code=400, detail="camera must be 'entry' or 'exit'")

    if not rtsp_url:
        raise HTTPException(status_code=400, detail=f"RTSP URL for {camera} camera is not configured")
